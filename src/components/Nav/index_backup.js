...
function Navigation(props) {
    ...
    useEffect(() => {
      ...
      } else if (!validateEmailUtil(email)) {
      ...
      if (authMode === 'signup' && loginPassword) {
        const passwordStrength = checkPasswordStrength(loginPassword);
        if (!passwordStrength.valid) {
          errs.push('Password must contain at least 3 of: uppercase, lowercase, numbers, symbols');
        }
      }
      ...
      try {
        ...
        if (authMode === 'signup') {
          ...
          const userData = {
            ...
            isAdmin: loginEmail.includes('admin') || loginEmail.includes('colin') || firstName.toLowerCase().includes('colin'),
          };

          // Create user with validation system
          ...
          showNotification('Account created! Please check your email for verification.', 'info', 5000, {
            category: 'account',
            icon: '📧'
          });
          ...
          setTimeout(() => {
            const existingUser = UserManager.getUserByEmail(loginEmail);
            
            if (!existingUser) {
              setLoginMsg('Account not found. Please check your email or create a new account.');
              setLoginBusy(false);
              return;
            }

            // Check if user can login
            const loginCheck = canUserLogin(existingUser);
            if (!loginCheck.allowed) {
              if (loginCheck.reason === 'email_not_verified') {
                ...
                setLoginMsg(loginCheck.message);
                setLoginBusy(false);
                return;
              }
            }
            ...
            if (existingUser.passwordHash !== loginPassword && !isAdminLogin) {
            ...
            // Successful login
            const user = isAdminLogin ? { ...existingUser, isAdmin: true, role: 'administrator' } : existingUser;
    ...
    // Original signup logic preserved for backward compatibility
